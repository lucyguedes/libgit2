...
#include "khash.h"
__KHASH_TYPE(line, git_oid, uint16_t);
typedef khash_t(line) blame_linemap;
GIT_INLINE(khint_t) linemap_hash(git_oid oid)
{
	khint_t h;
	memcpy(&h, &oid, sizeof(khint_t));
	return h;
}
GIT_INLINE(bool) linemap_equal(git_oid a, git_oid b)
{
	return git_oid_equal(&a, &b);
}
#define GIT__USE_LINEMAP \
	__KHASH_IMPL(line, static kh_inline, git_oid, uint16_t, 1, linemap_hash, linemap_equal)
